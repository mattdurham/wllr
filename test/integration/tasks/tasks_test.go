...
func call(t *testing.T, ctx context.Context, host *extension.Host, name string, params any) map[string]any {
	...
	if result.IsError {
		t.Fatalf("%s: %s", name, result.Result)
	}
...
func TestDurableTaskLifecycleAndReplay(t *testing.T) {
	...
	if int64(claimed["version"].(float64)) <= version {
		t.Fatal("claim did not advance version")
	}
	...
	if reported["status"] != "completed" {
		t.Fatalf("status = %v", reported["status"])
	}
	events := call(t, ctx, host, "tasks_events_after", map[string]any{"list_id": listID, "cursor": 0, "limit": 100})["events"].([]any)
	if len(events) < 4 {
		t.Fatalf("got %d events, want list/create/claim/report", len(events))
	}
	seen := map[string]bool{}
	for _, raw := range events {
		id := raw.(map[string]any)["event_id"].(string)
		if seen[id] {
