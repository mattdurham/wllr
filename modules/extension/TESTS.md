...
**Scenario:** The pure decision step of `DispatchEventChain`.
**Assertions:** observe leaves the event unchanged; a non-empty `Payload`
threads the new payload (type preserved); `Block` and `Cancel` stop the chain;
an empty `Error` falls back to `"blocked by extension <name>"`.

#### TestBlockReason

**Scenario:** User-facing blocked-tool-call text.
**Assertions:** empty reason → `"tool call blocked by extension"`; non-empty →
`"tool call blocked: <reason>"`.

#### TestRunBeforeToolCall_NoInterceptorsKeepsInput

**Scenario:** With no subscribed interceptors, `runBeforeToolCall` returns the
original input, not blocked.

#### TestExecuteTool_NativeToolReceivesInput
