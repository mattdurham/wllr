...
func init() {
	registerPlanTool("plan_create", "Create a durable plan with optional ordered steps.", `{"type":"object","properties":{"title":{"type":"string"},"description":{"type":"string"},"content":{"type":"string"},"steps":{"type":"array","items":{"type":"object","properties":{"id":{"type":"string"},"title":{"type":"string"},"description":{"type":"string"},"acceptance_checks":{"type":"array","items":{"type":"string"}}},"required":["title"]}}},"required":["title"]}`)
	...
	registerPlanTool("plan_update", "Update plan metadata or content. Supplied empty strings clear fields.", `{"type":"object","properties":{"id":{"type":"string"},"title":{"type":"string"},"description":{"type":"string"},"status":{"type":"string","enum":["active","paused","completed","archived"]},"content":{"type":"string"}},"required":["id"]}`)
	registerPlanTool("plan_step_update", "Update a plan step's status, notes, or description.", `{"type":"object","properties":{"plan_id":{"type":"string"},"step_id":{"type":"string"},"status":{"type":"string","enum":["pending","in_progress","completed","blocked"]},"description":{"type":"string"},"notes":{"type":"string"}},"required":["plan_id","step_id"]}`)
	...
	registerPlanTool("plan_complete", "Complete a plan only when every step is completed.", `{"type":"object","properties":{"plan_id":{"type":"string"}},"required":["plan_id"]}`)
...
func loadPlanState() {
	...
	state = emptyPlanState()
	if raw, ok := StoreGet(planStoreKey); ok && raw != "" {
		var saved planState
		if err := json.Unmarshal([]byte(raw), &saved); err == nil && saved.Plans != nil {
			state = saved
		}
	}
	stateReady = true
...
func planUpdate(input json.RawMessage) (string, bool) {
	...
	return result(map[string]any{"id": plan.ID, "title": plan.Title, "status": plan.Status, "message": "plan updated"})
}
...
func planComplete(input json.RawMessage) (string, bool) {
	...
	if missing := incompleteSteps(plan); len(missing) > 0 {
		out, _ := result(map[string]any{"completed": false, "incomplete_steps": missing})
		return out, true
	}
	...
	return result(map[string]any{"completed": true, "plan_id": plan.ID, "status": plan.Status})
}
